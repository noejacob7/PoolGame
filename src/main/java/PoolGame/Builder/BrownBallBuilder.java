...
import PoolGame.Observer.BrownBallCheat;
...
public class BrownBallBuilder implements BallBuilder {
     ...
     */
    public BrownBallBuilder(BallType type, BallPocketStrategy action, Integer scoreIncrement) {
        this.ballType = type;
        this.action = action;
    ...
    public Ball finaliseBuild() {
        Ball ball = this.ball;
        BrownBallCheat.getInstance().attach(ball);
        this.reset();
        return ball;
    }
}
