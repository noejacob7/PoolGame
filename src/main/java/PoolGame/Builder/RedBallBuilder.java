...
import PoolGame.Observer.RedBallCheat;
...
public class RedBallBuilder implements BallBuilder {
     ...
     */
    public RedBallBuilder(BallType type, BallPocketStrategy action, Integer scoreIncrement) {
        this.ballType = type;
        this.action = action;
    ...
    public Ball finaliseBuild() {
        Ball ball = this.ball;
        RedBallCheat.getInstance().attach(ball);
        this.reset();
        return ball;
    }
}
