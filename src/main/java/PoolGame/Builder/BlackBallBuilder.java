...
import PoolGame.Observer.BlackBallCheat;
...
public class BlackBallBuilder implements BallBuilder {
     ...
     */
    public BlackBallBuilder(BallType type, BallPocketStrategy action, int scoreIncrement) {
        this.ballType = type;
        this.action = action;
    ...
    public Ball finaliseBuild() {
        Ball ball = this.ball;
        BlackBallCheat.getInstance().attach(ball);
        this.reset();
        return ball;
    }
}
