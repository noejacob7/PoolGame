...
import PoolGame.Observer.YellowBallCheat;
...
public class YellowBallBuilder implements BallBuilder {
     ...
     */
    public YellowBallBuilder(BallType type, BallPocketStrategy action, Integer scoreIncrement) {
        this.ballType = type;
        this.action = action;
    ...
    public void reset() {
        ...
        if (this.scoreIncrement != null){
            this.ball.setScoreIncrement(this.scoreIncrement);
        }
    }
    ...
    public Ball finaliseBuild() {
        Ball ball = this.ball;
        YellowBallCheat.getInstance().attach(ball);
        this.reset();
        return ball;
    }
}
