...
import PoolGame.Observer.PurpleBallCheat;
...
public class PurpleBallBuilder implements BallBuilder {
     ...
     */
    public PurpleBallBuilder(BallType type, BallPocketStrategy action, Integer scoreIncrement) {
        this.ballType = type;
        this.action = action;
    ...
    public Ball finaliseBuild() {
        Ball ball = this.ball;
        PurpleBallCheat.getInstance().attach(ball);
        this.reset();
        return ball;
    }
}
