...
import PoolGame.Observer.GreenBallCheat;
...
public class GreenBallBuilder implements BallBuilder {
     ...
     */
    public GreenBallBuilder(BallType type, BallPocketStrategy action, Integer scoreIncrement) {
        this.ballType = type;
        this.action = action;
    ...
    public Ball finaliseBuild() {
        Ball ball = this.ball;
        GreenBallCheat.getInstance().attach(ball);
        this.reset();
        return ball;
    }
}
