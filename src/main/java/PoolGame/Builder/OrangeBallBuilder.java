...
import PoolGame.Observer.OrangeBallCheat;
...
public class OrangeBallBuilder implements BallBuilder {
     ...
     */
    public OrangeBallBuilder(BallType type, BallPocketStrategy action, Integer scoreIncrement) {
        this.ballType = type;
        this.action = action;
    ...
    public Ball finaliseBuild() {
        Ball ball = this.ball;
        OrangeBallCheat.getInstance().attach(ball);
        this.reset();
        return ball;
    }
}
